...
SC_MODULE(ula_rst)
{
    ...
    //+--------------------------------------------------------------------------
    SC_HAS_PROCESS(ula_rst);
     ula_rst(sc_module_name name, ula_rst_interface* sc_if) : sc_module(name)
    {
...
//+--------------------------------------------------------------------------
void ula_rst::wait_posedge(int n_time)
{
...
//+--------------------------------------------------------------------------
void ula_rst::wait_negedge(int n_time)
{
   for(int i = 0; i < n_time; i++) {
      trap_negedge();
   }
...
//+--------------------------------------------------------------------------
void ula_rst::sync_reset(int time_hi=5, int time_low=2)
{
    wait(1);
...
//! for more 2 clocks and spendy 1 clock in high level.
//+--------------------------------------------------------------------------
void ula_rst::async_reset(bool fast_reset=0)
{
